...
const version = "Fina Ergen v 3.5.2";
const buildDate = "Dom 15 Feb 2026 14:22";
...
const getSystemStats = async () => {
    const pyPath = "/home/claudio/.venv/bin/python3";
    const scriptPath = "/home/claudio/Descargas/Fina-Ergen/plugins/system/get_stats.py";
    try {
...
const handleSentinelCommand = async () => {
    ...
    if (cmd === 'help' || cmd === 'ayuda' || cmd === '?') {
        ...
        try {
            const py = "/home/claudio/.venv/bin/python3";
            const script = "/home/claudio/Descargas/Fina-Ergen/plugins/system/network_guardian.py";
...
const logVoice = async (cmd) => {
    try {
        const py = "/home/claudio/.venv/bin/python3";
        const script = "/home/claudio/Descargas/Fina-Ergen/plugins/system/log_manager.py";
...
const startSentinelDemo = () => {
    ...
    setInterval(() => {
        ...
        const ips = ["192.168.1.105", "10.0.0.5", "45.33.22.11", "201.55.42.99", "172.16.0.4"];
...
const fetchRecentEmails = async () => {
    try {
        const py = "/home/claudio/.venv/bin/python3";
        const script = "/home/claudio/Descargas/Fina-Ergen/scripts/mail_reader.py";
        const output = await invoke("execute_shell_command", { command: `${py} "${script}"` });
        const res = JSON.parse(output);
        if (res.status === 'success') {
...
const sendMobileSMS = async (number, msg) => {
    // Ensure we have a linked device
    ...
    finaState.value.process = `CONECTANDO ${dev.name.toUpperCase()}...`;

    try {
        const py = "/home/claudio/.venv/bin/python3";
        const script = "/home/claudio/Descargas/Fina-Ergen/plugins/system/mobile_hub.py";

        // 1. Try Connection (Blocking wait)
        if (dev.ip) {
            await invoke("execute_shell_command", { command: `timeout 3 adb connect ${dev.ip}` }).catch(() => { });
        }

        // 2. Check connectivity
        const devicesOut = await invoke("execute_shell_command", { command: "adb devices" });
        // Check if listed AND not offline/unauthorized
        ...
        finaState.value.process = "ENVIANDO SMS...";
        // Send SMS
        await invoke("execute_shell_command", { command: `${py} "${script}" send_sms --ip ${dev.ip} --number "${number}" --msg "${msg}"` });
        notifyFina("SMS ENVIADO");
    } catch (e) {
        console.error("SMS Error:", e);
        mobileHubError.value = `⚠️ Error: ${dev.name} desconectado. Conecte USB y reintente.`;
        setTimeout(() => mobileHubError.value = null, 8000);

        // UI Feedback (No usar notifyFina para evitar race conditions)
        finaState.value.process = `CONECTE ${dev.name} X USB`;

        // Voice Feedback
        const hint = `No puedo conectar con ${dev.name}. Conectalo por USB para reactivar la depuración.`;
        invoke("execute_shell_command", {
            command: `/home/claudio/.venv/bin/python3 /home/claudio/Descargas/Fina-Ergen/utils.py say "${hint}"`
        }).catch(() => { });

        // Reset UI later
        setTimeout(() => {
            if (finaState.value.process.includes("CONECTE")) {
...
const initiateMobileCall = async (number) => {
    ...
    try {
        ...
        await invoke("execute_shell_command", { command: `${py} "${script}" make_call --ip ${dev.ip} --number "${number}"` });
        ...
        finaState.value.process = `CONECTE ${dev.name} X USB`;
        const hint = `Fallo de conexión con ${dev.name}. Conectalo por USB para reactivar la depuración.`;
...
const scanNetwork = async () => {
    ...
    try {
        const cmd = "/home/claudio/Descargas/Fina-Ergen/iot/network_scan.py";
...
const sendCommAction = async () => {
    ...
    if (commMode.value === 'sms') {
        ...
        await sendMobileSMS(commTarget.value, commBody.value);
    } else {
        await initiateMobileCall(commTarget.value);
    }
};
...
const handleMessagingClick = () => {
    if (linkedMobileDevice.value) {
        ...
        finaState.value.process = "VINCULA TU CELULAR";
        activeTab.value = 'ajustes';
        activeSettingsTab.value = 'plugins';
        // Force navigate to "Nódulos de Conectividad"
        activeSettingsDomain.value = 'inteligencia';
        setTimeout(() => {
            const el = document.getElementById('nodulos-section');
...
const updateWeather = async () => {
    try {
        ...
        const pyPath = "/home/claudio/.venv/bin/python3";
        const scriptPath = "/home/claudio/Descargas/Fina-Ergen/iot/clima_api.py";

        // Ejecutar script con argumentos
        const jsonStr = await invoke("execute_shell_command", { command: `${pyPath} "${scriptPath}" "${apiKey}" "${cityId}"` });
...
const weatherIcon = computed(() => {
    ...
    testDoorbell = async () => {
        ...
        const pyPath = "/home/claudio/.venv/bin/python3";
        const scriptPath = "/home/claudio/Descargas/Fina-Ergen/plugins/doorbell/monitor.py";
...
const hangUp = async () => {
    try {
        ...
        const pyPath = "/home/claudio/.venv/bin/python3";
        const hangupScript = "/home/claudio/Descargas/Fina-Ergen/plugins/doorbell/hangup_doorbell.py";
...
const refreshAcStatus = async (silent = false) => {
    const pyPath = "/home/claudio/.venv/bin/python3";
    const scriptPath = "/home/claudio/Descargas/Fina-Ergen/iot/clima.py";
...
const refreshDoorbellStatus = async () => {
    const pyPath = "/home/claudio/.venv/bin/python3";
    const scriptPath = "/home/claudio/Descargas/Fina-Ergen/plugins/doorbell/doorbell_status.py";
...
const fetchSettings = async () => {
    try {
        const pyPath = "/home/claudio/.venv/bin/python3";
        const script = `import json, os; print(json.dumps(json.load(open("/home/claudio/Descargas/Fina-Ergen/config/settings.json"))))`;
...
const importFromCore = async () => {
    ...
    try {
        const rootPath = "/home/claudio/Descargas/Fina-Ergen";
...
const saveSettings = async () => {
    ...
    try {
        ...
        const pyPath = "/home/claudio/.venv/bin/python3";
        const scriptPath = "/home/claudio/Descargas/Fina-Ergen/iot/save_settings.py";
...
const sendTvCommand = async (script, args = "") => {
    ...
    const pyPath = "/home/claudio/.venv/bin/python3";
    const tvScript = `/home/claudio/Descargas/Fina-Ergen/plugins/tv/${modelFolder}/${script}`;
...
const tvDeco = () => {
    // Si estamos en la habitación Deco, enviamos el comando al Deco
    // Pero el usuario quiere que el botón HDMI cambie la entrada de la TV actual a HDMI
    // La TV actual es la que está en activeTvRoom.
};
...
onMounted(async () => {
    ...
    setInterval(getSystemStats, 5000); // Polling cada 5s
    getSystemStats(); // Llama inmediata
    await fetchSettings(); // Cargo settings primero y espero a que termine
    // await loadTvChannels(); // DESACTIVADO: Se carga on-demand al abrir tab TV
    updateWeather(); // Ahora sí clima tiene los IDs

    // SECUENCIA DE ARRANQUE PREMIUM [ESTRICTO]
    try {
        finaState.value.process = "CARGANDO SISTEMA";
        await new Promise(r => setTimeout(r, 2000)); // Un poco más rápido

        finaState.value.process = "CORROBORANDO ESTADOS";
        await new Promise(r => setTimeout(r, 1000));

        // 1. AIRE
        finaState.value.process = "SINCRONIZANDO AIRE SURREY...";
        try {
            await refreshAcStatus();
            const acMsg = `AIRE ${acState.value.power ? 'ENCENDIDO' : 'APAGADO'} EN
                                    ${acState.value.mode.toUpperCase()} A ${acState.value.temp}°`;
            finaState.value.process = acMsg;
        } catch (e) {
            finaState.value.process = "ERROR AL SINCRONIZAR AIRE";
        }
        await new Promise(r => setTimeout(r, 2500));

        // 2. TIMBRE
        finaState.value.process = "SINCRONIZANDO TIMBRE TUYA...";
        try {
            await refreshDoorbellStatus();
            const doorbellMsg = `TIMBRE CARGADO - BATERÍA ${doorbellBattery.value}%`;
            finaState.value.process = doorbellMsg;
        } catch (e) {
            finaState.value.process = "ERROR TIMBRE";
        }
        await new Promise(r => setTimeout(r, 2500));

        // 3. NODOS TV (OPTIMIZADO)
        // Ya no matamos ADB ni conectamos a todo el mundo a lo bruto.
        // Solo verificamos la TV de la habitación actual si es necesario.

        // 3. NODOS TV (ESCANEO ACTIVO E INTELIGENTE)
        finaState.value.process = "ESCANEANDO RED DE TVS...";

        const tvList = userSettings.value.tvs || [];
        let detectedTv = null;

        // Intentar conectar a TODAS las TVs configuradas para ver cuál responde
        for (const tv of tvList) {
            if (tv.enabled !== false && tv.ip) {
                try {
                    await invoke("spawn_shell_command", { command: `timeout 0.5 adb connect ${tv.ip}` }).catch(() => { });
                } catch (e) { }
            }
        }

        // Verificar cuáles quedaron conectadas realmente
        try {
            const devicesOut = await invoke("spawn_shell_command", { command: "adb devices" });
            console.log("📡 ADB Scan Result:", devicesOut);
            ...
            // Buscar la primera coincidencia en la lista de settings
            for (const tv of tvList) {
                if (tv.ip && devicesOut.includes(tv.ip)) {
                    detectedTv = tv;
                    break; // Encontramos una activa, nos quedamos con esa
                }
            }
        } catch (e) {
        ...
        if (detectedTv) {
            activeTvRoom.value = detectedTv.room;
            activeTvIp.value = detectedTv.ip;
            finaState.value.process = `TV CONECTADA: ${detectedTv.name || detectedTv.ip}`;
            console.log(`✅ Fina Auto-Detect: TV ${detectedTv.name || detectedTv.room} está online.`);
        } else {
            // Fallback: Usar la primera de la lista (Dormitorio por defecto según settings.json)
            const fallbackTv = tvList[0];
            if (fallbackTv) {
                activeTvRoom.value = fallbackTv.room;
                activeTvIp.value = fallbackTv.ip;
                finaState.value.process = `TV DEFAULT: ${fallbackTv.name || fallbackTv.ip}`;
            }
        }
        ...
        finaState.value.process = `CONECTADOS ${adbDevices.value.length} NODOS TV`;
        await new Promise(r => setTimeout(r, 1000));

        // 4. SALUDO FINAL
        const greeting = getGreeting();
        finaState.value.process = greeting.toUpperCase();
    ...
    // CARGA REAL SILENCIOSA (TEMPORAL)
    try { refreshAcStatus(true); refreshDoorbellStatus(); } catch (e) { }
    finaState.value.process = "INICIADO (ESPERANDO DATOS...)";

    // Configurar intervalos posteriores
    updateClock();
    setInterval(updateClock, 1000);
    updateWeather();
    ...
    setInterval(async () => {
        try {
            ...
            // Mostrar error rápido (tras 5 fallos = 0.5 seg)
            if (window.pollErrors > 5) {
                const errDetail = `${e.name}: ${e.message}`;
                console.error("API Error Poll:", e);
                finaState.value.process = "ERR RED: " + errDetail;
                finaState.value.status = "offline"; // Poner icono o estado offline visualmente
            }
        }
    }, 1000);
        ...
        try {
            ...
            if (data.type === 'event' && data.name === 'doorbell-ring') {
                ...
                if (window.lockTextUntil && Date.now() < window.lockTextUntil &&
                    ...
                    if (finaState.value.process !== "SISTEMA LISTO" && finaState.value.process !== "ESUCHANDO..." && finaState.value.process !== "Diga 'Fina' para empezar") {
                        window.lockTextUntil = Date.now() + 3000;
                    }
                }
...
const loadTvChannels = async () => {
    try {
        ...
        const script = `import json, os; print(json.dumps(json.load(open("/home/claudio/Descargas/Fina-Ergen/config/${channelFile}"))))`;
        const output = await invoke("execute_shell_command", { command: `${pyPath} -c '${script}'` });
...
const scanChannels = async () => {
    ...
    const pyPath = "/home/claudio/.venv/bin/python3";
    const scriptPath = `/home/claudio/Descargas/Fina-Ergen/plugins/tv/${modelFolder}/scan_ultra_fast.py`;
    try {
...
const launchTvApp = async (pkg) => {
    ...
    const pyPath = "/home/claudio/.venv/bin/python3";
    const scriptPath = `/home/claudio/Descargas/Fina-Ergen/plugins/tv/${modelFolder}/launch_app.py`;
...
const scanTvApps = async () => {
    ...
    const pyPath = "/home/claudio/.venv/bin/python3";
    const scriptPath = `/home/claudio/Descargas/Fina-Ergen/plugins/tv/${modelFolder}/list_tv_apps.py`;
...
const enrollVoice = () => {
    ...
    const interactive = "/home/claudio/Descargas/Fina-Ergen/plugins/biometria/run_interactive.sh";
    const py = "/home/claudio/.venv/bin/python3";
    const script = "/home/claudio/Descargas/Fina-Ergen/plugins/biometria/train_voice.py";
...
const enrollFace = () => {
    ...
    const interactive = "/home/claudio/Descargas/Fina-Ergen/plugins/biometria/run_interactive.sh";
    const py = "/home/claudio/.venv/bin/python3";
    const script = "/home/claudio/Descargas/Fina-Ergen/plugins/biometria/train_face.py";
    invoke("spawn_shell_command", { command: `${interactive} "${py} ${script}"` }).catch(e => console.error(e));
};
...
const enrollFinger = () => {
    ...
    const interactive = "/home/claudio/Descargas/Fina-Ergen/plugins/biometria/run_interactive.sh";
    const py = "/home/claudio/.venv/bin/python3";
    const script = "/home/claudio/Descargas/Fina-Ergen/plugins/biometria/enroll_finger.py";
    invoke("spawn_shell_command", { command: `${interactive} "${py} ${script}"` }).catch(e => console.error(e));
};
...
<template>
    <div class="flex h-screen w-screen overflow-hidden bg-[#010409] font-sans text-slate-100 selection:bg-cyan-500/20">
        ...
        <main
            ...
            <div class="absolute inset-0 z-0 flex flex-col items-center justify-center pointer-events-none pb-20">
                <!-- CHAT BUBBLES -->
                <div class="absolute top-20 right-10 flex flex-col gap-3 items-end max-w-xs pointer-events-none">
                    <transition-group name="list">
                        <div v-for="msg in chatHistory" :key="msg.id"
                            ...
                            <p class="text-[11px] text-cyan-100 font-medium lowercase">
                                {{ msg.text }}</p>
                    ...
                    <transition name="fade" mode="out-in">
                        ...
                        <div v-if="showSentinel"
                            ...
                            <div
                                ...
                                <div class="flex items-center gap-6">
                                    <div class="relative">
                                        <i
                                            class="fa-solid fa-shield-cat text-4xl text-red-500 animate-pulse relative z-10"></i>
                                    ...
                                    <div class="flex flex-col">
                                        ...
                                        <div class="flex items-center gap-3">
                                            <span
                                                class="text-xs font-mono text-red-400 uppercase tracking-[0.4em]">Sistema
                            ...
                            <div class="flex-1 flex overflow-hidden">
                                ...
                                <div
                                    ...
                                    <div class="text-center z-20">
                                        <div
                                            ...
                                            <span
                                                class="text-xs font-black text-red-500 uppercase tracking-widest">Escaneando
                                                Red</span>
                                        </div>
                                ...
                                <div class="w-[30%] border-r border-white/5 p-10 flex flex-col gap-8 bg-black/20">
                                    ...
                                    <!-- Histogram -->
                                    <div
                                        class="h-40 bg-black/40 rounded-2xl border border-white/5 p-6 flex items-end justify-between gap-1.5 shadow-inner">
                                        <div v-for="i in 16" :key="i"
                                            class="w-full bg-gradient-to-t from-red-900/60 to-red-500/60 hover:to-red-400 transition-all rounded-sm"
                                    ...
                                    <div class="grid grid-cols-2 gap-4">
                                        ...
                                        <div
                                            ...
                                            <div class="flex flex-col items-center leading-none">
                                                ...
                                                <span class="text-[11px] text-cyan-400 font-bold">↓ {{
                                    ...
                                    <div class="grid grid-cols-1 gap-4">
                                        <div
                                            ...
                                            <div class="flex flex-col">
                                                ...
                                                <span
                                                    class="text-xs font-bold text-slate-500 uppercase tracking-wider">IPs
                                                    Bloqueadas</span>
                                            </div>
                                            <i
                                    ...
                                    <div class="mt-auto p-6 bg-yellow-500/5 border border-yellow-500/10 rounded-2xl">
                                        ...
                                        <div class="h-2 w-full bg-slate-800 rounded-full overflow-hidden">
                                            <div
                                                class="h-full w-[35%] bg-gradient-to-r from-yellow-600 to-yellow-400 animate-pulse">
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <!-- RIGHT PANEL: LOGS -->
                                <div
                                    class="w-[30%] bg-[#020205] flex flex-col font-mono text-xs border-l border-white/5">
                                    <div
                                        class="h-14 flex items-center px-6 border-b border-white/5 bg-white/5 justify-between">
                                    ...
                                    <div class="p-4 border-t border-white/10 bg-white/5">
                                        <div class="flex gap-3 items-center">
                                            ...
                                            <input v-model="sentinelInput" @keyup.enter="handleSentinelCommand"
                                                type="text" placeholder="Ejecutar contramedida..."
                            ...
                            <div @click="testDoorbell"
                                ...
                                <div
                                    ...
                                    <span class="text-[10px] font-black text-slate-300 uppercase leading-none">{{
                                        doorbellBattery }}% BATERÍA</span>
                                </div>
                            </div>
                        </div>
                        <div v-if="activeTab === 'clima'" class="w-full flex flex-col items-center">
                        ...
                        <div v-if="activeTab === 'agenda'"
                            ...
                            <div class="w-full max-w-7xl flex flex-col items-center shrink-0">
                                ...
                                <div class="w-full grid grid-cols-1 lg:grid-cols-3 gap-8 items-center">
                                    ...
                                    <div
                                        ...
                                        <div class="space-y-4">
                                            <div
                                                class="flex flex-col border-l-2 border-cyan-500/50 pl-4 py-1 hover:bg-white/5 rounded-r-lg transition-colors cursor-pointer">
                                                <span class="text-[11px] font-bold text-white">Enviar reporte
                                                    final</span>
                                                <span
                                                    class="text-[9px] text-slate-500 uppercase font-black tracking-widest mt-0.5">Prioritariamente</span>
                                            </div>
                                            <div
                                                class="flex flex-col border-l-2 border-purple-500/50 pl-4 py-1 hover:bg-white/5 rounded-r-lg transition-colors cursor-pointer">
                                                <span class="text-[11px] font-bold text-white">Pagar servicio</span>
                                                <span
                                                    class="text-[9px] text-slate-500 uppercase font-black tracking-widest mt-0.5">Anteriormente</span>
                                            </div>
                                            <div
                                                class="flex flex-col border-l-2 border-pink-500/50 pl-4 py-1 hover:bg-white/5 rounded-r-lg transition-colors cursor-pointer">
                                                <span class="text-[11px] font-bold text-white">Llamar al banco</span>
                                                <span
                                                    class="text-[9px] text-slate-500 uppercase font-black tracking-widest mt-0.5">Anteriormente</span>
                                            </div>
                                        </div>
                                    ...
                                    <div
                                        ...
                                        <div class="flex items-center justify-between">
                                            ...
                                            <div @click="fetchRecentEmails"
                                                class="cursor-pointer hover:rotate-180 transition-transform duration-500">
                                                <i class="fa-solid fa-arrows-rotate text-[10px] text-slate-600"></i>
                                            </div>
                                        </div>
                                        <div class="space-y-4">
                                            ...
                                            <div v-if="recentEmails.length === 0"
                                                class="flex flex-col border-r-2 border-slate-500/50 pr-4 py-1 items-end text-right">
                                                <span class="text-[11px] font-bold text-slate-400">Sin asuntos</span>
                                                <span
                            ...
                            <div
                                ...
                                <div class="flex items-center justify-between mb-6">
                                    <h4 class="text-sm font-black text-white uppercase tracking-[0.2em]">Comunicación
                                    </h4>
                                    <div class="flex items-center gap-2">
                                        <span class="text-[9px] font-black text-slate-500">SÍNC.</span>
                                ...
                                <div class="grid grid-cols-2 gap-8">
                                    <div @click="handleMessagingClick"
                                        ...
                                        <div class="flex flex-col">
                                            <span class="text-sm font-black text-white uppercase">Mensajería</span>
                                            <span
                        ...
                        <div v-if="activeTab === 'ajustes'"
                            ...
                            <div v-else
                                                ...
                                                <div class="mt-8 text-center z-10">
                                                    <div class="flex items-center gap-6 justify-center">
                                                        ...
                                                        <div class="flex flex-col items-center">
                                                            ...
                                                            <span
                                                                class="text-[9px] font-black text-indigo-400 uppercase tracking-widest mt-2">Revisión
                                                                Matrix</span>
                                                        </div>
                                                    </div>
                                                </div>
                                        ...
                                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-10">
                                            ...
                                            <div class="space-y-10">
                                                ...
                                                <div
                                                    ...
                                                    <span
                                                        class="text-[10px] font-black text-indigo-400 uppercase tracking-[0.3em] block border-b border-indigo-500/10 pb-4 italic">Heurísticas
                                                        de Sistema</span>
                                        ...
                                        <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
                                                        ...
                                                        <transition name="fade">
                                                            <div v-if="assigningDeviceIp === dev.ip"
                                                                ...
                                                                <div class="grid grid-cols-2 gap-4">
                                                                    ...
                                                                    <div class="space-y-1">
                                                                        <label
                                                                            class="text-[8px] font-black text-slate-500 uppercase ml-2">Habitáculo
                                                                            (Solo TV)</label>
                                            ...
                                            <div class="space-y-6">
                                                ...
                                                <div
                                                    ...
                                                    <div class="flex flex-col">
                                                        <span
                                                            class="text-sm font-black text-white uppercase tracking-widest">Migración
                                                            Core</span>
                                                        <span
                                                ...
                                                <div
                                                    ...
                                                    <div class="flex flex-col">
                                                        <span
                                                            class="text-sm font-black text-white uppercase tracking-widest">Depuración
                                                            ADB</span>
                                                        <span
                                        ...
                                        <div
                                            ...
                                            <div class="space-y-6">
                                                ...
                                                <div v-if="activeTvIp && tvStatuses[activeTvIp]" class="space-y-4">
                                                    <span
                                                        class="text-[10px] font-black text-slate-500 uppercase tracking-widest block mb-4">Configuración
                                                        de Apps</span>
                                                    <div class="grid grid-cols-2 gap-4">
                                ...
                                <!-- Cierre del layout interno del dominio -->
                            </div>
                            <!-- Cierre del contenedor principal del hub -->
                        </div> <!-- Cierre del tab Ajustes -->
                    </transition>
                </div>
            </div>
                ...
                <div v-if="showCredits"
                    ...
                    <div
                        ...
                        <div class="p-10 space-y-8">
                            ...
                            <button @click="showCredits = false"
                                class="w-full h-14 rounded-3xl bg-cyan-500 text-[#020617] font-black text-xs uppercase tracking-widest shadow-xl shadow-cyan-900/30">Cerrar
                                Sesión</button>
                        </div>
                    </div>
                </div>
                ...
                <div v-if="showCommModal"
                    ...
                    <div class="relative w-full max-w-lg bg-slate-900 border border-white/10 rounded-[30px] p-8 shadow-2xl flex flex-col gap-6"
                        ...
                        <div class="grid grid-cols-2 gap-4 mt-2">
                            ...
                            <button @click="sendCommAction"
                                class="py-4 rounded-2xl text-black text-xs font-black uppercase transition-all tracking-widest shadow-lg flex items-center justify-center gap-2"
                                :class="commMode === 'sms'
